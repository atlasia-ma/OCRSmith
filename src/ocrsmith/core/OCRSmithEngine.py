    ...
    def generate_sample(self, text: str, output_size: tuple = (800, 600)):
        ...
        if self.config.augmentations:
            try:
                ...
                pipeline = [cfg for cfg in self.augmentation_manager.augmentation_configs if cfg.get('enabled', True)]
                ...
                for cfg in pipeline:
                    ...
                    for key, val in list(params.items()):
                        if isinstance(val, (list, tuple)):
                            lo, hi = (float(val[0]), float(val[1])) if len(val) == 2 else (float(val[0]), float(val[0]))
                            if hi < lo:
                                lo, hi = hi, lo
