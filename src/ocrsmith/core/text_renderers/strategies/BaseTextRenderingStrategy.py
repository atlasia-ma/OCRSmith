...
class BaseTextRenderingStrategy(TextRenderingStrategy):
    ...
    def _prepare_canvas(self, width: int, height: int):
    ...
    def _draw_character(self, draw, draw_mask, font, char, x, y, fill, stroke_fill, stroke_width, char_index):
        ...
        # Mask coloring logic
        mask_color = ((char_index + 1) // (255 * 255), (char_index + 1) // 255, (char_index + 1) % 255)
        draw_mask.text(
            (x, y), char, fill=mask_color, font=font, stroke_width=stroke_width, stroke_fill=stroke_fill
        )
