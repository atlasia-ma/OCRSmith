...
def main():
    ...
    parser.add_argument(
        ...
        dest="overrides",
        action="append",
        help="Override config, e.g. text_data.source_path=assets/texts.csv",
        default=[],
    )
    parser.add_argument(
        "--set-json", dest="overrides_json", type=str, default=None, help="JSON overrides for config"
    )
    ...
    engine.generate_test_dataset(
        num_samples=args.num_samples, output_dir=args.output_dir, workers=args.workers
    )
