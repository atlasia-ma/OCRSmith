...
from .fonts import FontCache, FontLoader
...
class FontManager:
    ...
    def _find_fonts_at_path(self, path: str | Path) -> list[Path]:
        validated_paths = []
    ...
    def _parse_font_configs(self) -> list[dict[str, Any]]:
        ...
        for font in self.config.fonts:
            ...
            for path in font_paths:
                ...
                configs.append({"font_name": font_name, "font_path": path, "size": font_config.get("size")})

        return configs

    def get_random_font(self, font_size: int | None = None) -> FreeTypeFont:
        if not self.font_configs:
    ...
    def get_font_by_name(self, name: str, font_size: int | None = None) -> FreeTypeFont:
        matching_configs = [cfg for cfg in self.font_configs if cfg["font_name"] == name]

        if not matching_configs:
            available_names = list({cfg["font_name"] for cfg in self.font_configs})
    ...
    def get_default_font(self, font_size: int | None = None) -> FreeTypeFont:

        return FontLoader.create_default_font(font_size or self.default_size)

    def get_available_font_names(self) -> list[str]:

        return sorted({cfg["font_name"] for cfg in self.font_configs})
    ...
    @staticmethod
    def get_text_height(font: FreeTypeFont, text: str | None = None) -> int:
        if not text:
            bbox = font.getbbox(
                "هوما مخبّين شي حاجة, أنا متيقّن!هومAgp"
            )  # Use representative chars for empty string height
